...
def auto_install_dependencies():
    ...
    """
    print(f"{Fore.CYAN}🛠️ Glockzada7 Toolkit - Dependency Installer{Style.RESET_ALL}")
    print("="*60)
...
class AdvancedNetworkTools:
    def __init__(self):
        ...
        if self.is_termux:
            ...
            self.save_dir = os.path.join(base_dir, "Glockzada7's Network")
        else:
            self.save_dir = os.path.join(os.getcwd(), "Glockzada7's Network")
    ...
    def run(self):
        while True:
            ...
            if CURSES_AVAILABLE and self.menu_style == 'list':
                sel = curses.wrapper(_draw_curses_menu, "Glockzada7 Network Tool (Lite)", options)
            else:
                print(f"\n{Fore.CYAN}   Glockzada7 TOOLKIT - SELECT BY NUMBER{Style.RESET_ALL}")
